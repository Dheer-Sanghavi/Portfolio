...
async function callGemini(prompt, maxTokens=2000) {
  ...
  for (const model of models) {
    ...
    try {
      ...
      if (!res.ok) { lastErr = `${model}: ${JSON.stringify(data.error?.message)}`; continue; }
...
async function generatePost(date, t) {
  const prompt = `You are a professional content writer for a digital marketing portfolio. Write authoritative, original blog posts. Australia-first audience but globally relevant. Rules: ~1000 words; structured intro + 3 H2 sections + conclusion; cite 2-3 credible sources by name only (MIT Technology Review, Search Engine Journal, Google Search Central Blog, Moz, CSIRO, Gartner, McKinsey, Harvard Business Review, Australian Financial Review, Wired, The Verge); no em dashes; no AI filler phrases. Return ONLY valid JSON, no markdown, no backticks, no extra text.

Write a ~1000 word blog post on: "${t.topic}". Category: ${t.category}. Focus keyword: "${t.kw}". Date: ${date}.

Return ONLY this JSON structure:
{"title":"SEO title under 65 chars","category":"${t.category}","focusKeyword":"${t.kw}","metaDescription":"under 160 chars","readingTime":"X min read","intro":"opening paragraph about 120 words","sections":[{"h2":"section heading","content":"about 200 words"},{"h2":"section heading","content":"about 200 words"},{"h2":"section heading","content":"about 200 words"}],"conclusion":"closing paragraph about 100 words","sources":["Publication name and context","Publication name and context"],"tags":["tag1","tag2","tag3","tag4"]}`;
...
async function generateTips(date) {
  const prompt = `Generate exactly 3 byte-sized tech tips for ${date}. Mix categories from: AI tools, SEO tactics, developer productivity, digital marketing quick wins. Each tip must be 1-2 sentences, genuinely useful, non-obvious, and relevant to Australian professionals. Return ONLY a valid JSON array, no markdown, no backticks, no extra text:
[{"tip":"tip text here","category":"AI"},{"tip":"tip text here","category":"SEO"},{"tip":"tip text here","category":"Dev"}]`;
...
export default async (req, context) => {
  ...
  if (missing.length) return new Response(`Missing env vars: ${missing.join(', ')}`, { status: 500 });
  ...
  try {
    ...
    return new Response(JSON.stringify({ success: true, title: post.title, url: `${SITE_URL}/blog/${slug}.html` }), { status: 200, headers: { 'Content-Type': 'application/json' } });
  } catch(err) {
